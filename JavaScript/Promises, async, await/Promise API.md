...
published:
created: 2026-06-05
description: In this note you will learn about methods in the `Promise` class.
tags:
  - javascript
---
...
`Promise.all` does nothing to cancel them, as there’s no concept of “cancellation” in promises. We can do so using `AbortController`, but it's not a part of the Promise API. Will cover that in some other note.
...
Similar to `Promise.all`, but waits only for the first settled (doesn't matte if is fulfilled or rejected) promise and gets its result (or error).
...
The method is used for compatibility, when a function is expected to return a promise. `Promise.resolve()` can make a normal function return a promise, so some other part of code can consume it, say using `.then`.
