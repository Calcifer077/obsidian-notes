...
A function that is a property of an object is called its _method_. So, here we've got a method `sayHi` of the object `user`.
...
sayHi(); // undefined
```
In this case `this` is `undefined` in strict mode. If we try to access `this.name`, there will be an error. In non-strict mode the value of `this` in such case will be the _global object_ (`window` in a browser). Usually such call is a programming error. If there's `this` inside a function, it expects to be called in a object context.
...
#### What is the result of accessing `ref`?
```js
function makeUser() {
  return {
    name: "John",
