...
1. There are many system symbols used by JavaScript which are accessible as `Symbol.*`. We can use them to alter some built-in behaviours. For instance, later in the tutorial we’ll use `Symbol.iterator` for [iterables](https://javascript.info/iterable), `Symbol.toPrimitive` to setup [object-to-primitive conversion](https://javascript.info/object-toprimitive) and so on.
