...
tags:
  - javascript
---
As we know from [Garbage collection](../Objects/Garbage%20collection.md), JS engine keeps a value in memory while it is "reachable" and can potentially be used.

Usually, properties of an object or elements of an array or another data structure are considered (kept in memory) reachable while that data structure is in memory.
...
`WeakMap` and `WeakSet` are fundamentally different in this aspect. They don't prevent garbage-collection of key objects. 

# WeakMap

The first difference between `Map` and `WeakMap` is that keys must be objects, not primitives values:
```js
...
If there are no references to a key (object), it will be removed from memory and from the map automatically.
...
If we're working with an object that "_belongs_" to another code, maybe a third-party library, and we would like to store some data associated  with it, which should only exist while the object is in alive - then `WeakMap` is exactly what's needed.
...
For instance, we are keeping visit counts for users. This information is stored in a map. When a user leaves (its object gets garbage collected), we don't want to store their visit count anymore.
...
Now, `john` object should be garbage collected, but remains in memory, due to it being a key in the map. We need to clean `visitsCountMap` when we remove users, otherwise it will grow in memory indefinitely. Such cleaning can become a tedious task in complex architectures.
...
We can store "_cache_" results from a function, so that future calls on the same object can reuse it.

To achieve this, we can use `Map` (not optimal):
