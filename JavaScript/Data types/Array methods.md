...
The [arr.map](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map) method is one of the most useful and often used. It calls the function for each element of the array and returns the array of results.
...
Did you notice anything strange in the outcome? The order became `1, 15, 2`. Incorrect. But why?
...
Suppose we are writing a messaging app, and the person enters the comma-delimited list of receivers: `John, Pete, Mary`. But for us an array of names would be much more comfortable than a single string. How to get it?
...
The methods [arr.reduce](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce) and [arr.reduceRight](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduceRight) does the same, but are a little bit more intricate. They are used to calculate a single value based on the array.
...
![](../../assets/reduce_function_array_methods_modern_javascript_tutorial.png)
...
The value of `thisArg` parameter becomes `this` for `func`. It lets you specify what `this` should refer to inside the callback function.
...
Arrow functions don't have their own `this`, so they ignore `thisArg`. They capture it lexically. **Lexically** means that the value of `this` is determined by **where the function is written in the code**, not by how it is called.
...
obj.regular(); // undefined (or global object in non-strict mode)
obj.arrow();   // 10
```
## Summary
