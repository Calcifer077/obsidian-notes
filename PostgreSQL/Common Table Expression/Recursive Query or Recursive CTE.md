---
title: Recursive Query or Recursive CTE
source: https://neon.com/postgresql/tutorial/recursive-query
created: 2026-07-19
tags:
  - database
  - postgresql
---
A [`Common Table Expression`](Common%20Table%20Expression%20-%20CTE.md) is a named temporary result set within a query. 

A recursive CTE allows us to perform recursion within a query using the `WITH RECURSIVE` syntax. A recursive CTE is often referred to as a recursive query.
...
How PostgreSQL execute a CTE:

```
Execute anchor query / member
        │
        ▼
 Store result in:
 • Final result
...
Let's create a sample table for this example:
...
Let's use a recursive CTE to find all subordinates of the manager with the id 2.
