...
source: https://neon.com/postgresql/tutorial/cte
created: 2026-07-19
tags:
  - database
  - postgresql
...
A CTE helps us enhance the readability of a complex query by breaking it down into smaller and more reusable parts.
...
- **CTE query**: This is a query that defines the CTE, which may include [`Joins`](../Join%20Tables/Joins.md), [`WHERE`](../Filtering%20Data/WHERE.md),[`GROUP BY`](../Group%20Data/GROUP%20BY.md) clauses, and other valid SQL constructs.
...
The following query joins two tables:
...
- Ability to create [recursive queries](https://neon.com/postgresql/tutorial/postgresql-recursive-query), which are queries that reference themselves. The recursive queries come in handy when you want to query hierarchical data such as organization charts.
- Use in conjunction with [window functions](https://neon.com/postgresql/postgresql-window-function). You can use CTEs in conjunction with window functions to create an initial result set and use another select statement to further process this result set.
