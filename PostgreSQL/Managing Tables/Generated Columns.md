...
source: https://neon.com/postgresql/tutorial/generated-columns
created: 2026-08-04
tags:
  - database
  - postgresql
...
A virtual generated column is like a [view](https://neon.com/postgresql/postgresql-views), whereas a stored generated column is similar to a [materialized view](https://neon.com/postgresql/postgresql-views/postgresql-materialized-views). Unlike a material view, PostgreSQL automatically updates data for stored generated columns.
...
- The expression can only use immutable functions and cannot involve [subqueries](https://neon.com/postgresql/tutorial/postgresql-subquery) or reference anything beyond the current row. For example, the expression cannot use the [CURRENT_TIMESTAMP](https://neon.com/postgresql/postgresql-date-functions/postgresql-current_timestamp) function.
...
```
net_price = list_price + (list_price * tax / 100)) - (list_price * discount / 100)
```

Now, if you insert rows into the table, `net_price` will be automatically generated:
