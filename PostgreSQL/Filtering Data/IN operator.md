...
Functionally, the `IN` operator is equivalent to combining multiple boolean expressions with the [`OR`](OR%20operator.md) opeartors:
```PostgreSQL
...
### 1) Using with a list of numbers
...
### 2) Using with a list of strings
We'll use the `actor` table from the [Quick Start - Settings things up](../Quick%20Start%20-%20Setting%20things%20up/Quick%20Start%20-%20Settings%20things%20up.md) section.
![](../../assets/actor_table_dvdrental_and_operator_neon_postgres_sql_tutorial.png)
...
In this example, the `payment_date` column has the type [`timestamp`](https://neon.com/postgresql/tutorial/postgresql-timestamp) that consists of both date and time parts.
...
To do that you use the `::` [cast operator](https://neon.com/postgresql/tutorial/postgresql-cast):
```PostgreSQL
payment_date::date
```
...
The `NOT IN`operator is equivalent to a combination of multiple boolean expressions with the [`AND`](AND%20operator.md) operators:
```PostgreSQL
...
The following query retrieves the same set of data but uses the not-equal (`<>`) and [`AND`](AND%20operator.md) operators:
```PostgreSQL
SELECT
  film_id,
  title
...
## Sources
[Neon - in](https://neon.com/postgresql/tutorial/in)
