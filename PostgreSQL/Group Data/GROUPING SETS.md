...
source: https://neon.com/postgresql/tutorial/grouping-sets
created: 2026-06-28
tags:
  - database
  - postgresql
...
Let’s get started by [creating a new table](https://neon.com/postgresql/tutorial/postgresql-create-table) called `sales` for the demonstration.
...
VALUES
    ...
    ('XYZ', 'Premium', 100),
    ('XYZ', 'Basic', 300)
RETURNING *; -- used to return those rows that were just inserted
```
...
Suppose you want to get all the grouping sets using a single query. To achieve this, you can use the [`UNION ALL`](https://neon.com/postgresql/tutorial/postgresql-union) to combine all the result sets of the queries above.
...
To make it more efficient, PostgreSQL provides the `GROUPING SETS` clause which is the subclause of the [`GROUP BY`](GROUP%20BY.md) clause.
...
To apply this syntax to the above example, you can use `GROUPING SETS` clause instead of the [`UNION ALL`](https://neon.com/postgresql/tutorial/postgresql-union) clause like this:
...
![](../../assets/Pasted%20image%2020260628090736.png) 
...
![PostgreSQL GROUPING SETS - GROUPING function in HAVING clause](https://neon.com/_next/image?url=%2Fpostgresqltutorial%2FPostgreSQL-Grouping-Sets-GROUPING-function-in-HAVING-clause.png&w=640&q=75&dpl=dpl_8qYzVCk2Qn6u8biDKSCXnwFdmPvG)

## Summary

- Use the `GROUPING SETS` to generate multiple grouping sets.