...
The `DISTINCT ON` clause retrieves the first unique entry from each column or combination of columns in a result set. It keeps only the first row of each set of rows where the given expressions evaluate to equal.
...
[Neon - distinct on](https://neon.com/postgresql/tutorial/distinct-on)
